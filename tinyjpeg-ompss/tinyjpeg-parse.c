...
int parse_JFIF(struct jpeg_parse_context *jpc, const unsigned char *stream)
{
  ...
  while (!sos_marker_found)
  {
    ...
    marker = *stream++; //first char after beginnning 0xffs ->
    chuck_len = be16_to_cpu(stream); //length of segment excluding APP0 marker
    next_chunck = stream + chuck_len;
    trace("parse_JFIF --- marker: %x, chuck_len: %x, next_chunck: %x\n", marker, chuck_len, next_chunck);
    switch (marker)
    {
      case SOF:
